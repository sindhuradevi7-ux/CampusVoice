    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-16 text-center">
        <div className="bg-white p-8 rounded-3xl border border-burgundy-200 shadow-warm">
          <h2 className="text-xl font-bold text-wine-900 mb-2">Complaint Not Found</h2>
          <p className="text-sm text-wine-600 mb-6 font-medium">{error || 'Unable to locate this complaint.'}</p>
          <Link
            to="/my-complaints"
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 pb-16">
      ...
      <div className="bg-white p-6 sm:p-8 rounded-3xl border border-cream-300 shadow-warm space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-cream-300 pb-5">
          ...
          <div className="text-right sm:text-right text-xs text-wine-600 space-y-1">
            ...
            <div className="text-burgundy-800 font-bold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 rounded-2xl bg-cream-50 border border-cream-300 shadow-sm">
            <span className="text-xs text-wine-500 font-bold block mb-1">Location</span>
            <div className="flex items-center gap-1.5 text-sm font-bold text-wine-900">
              <MapPin className="w-4 h-4 text-peach-700 shrink-0" />
              <span>{complaint.location}</span>
            </div>
            {complaint.affectedArea && (
              <span className="text-xs text-wine-500 font-medium block mt-1">Area: {complaint.affectedArea}</span>
            )}
          </div>

          <div className="p-4 rounded-2xl bg-cream-50 border border-cream-300 shadow-sm">
            <span className="text-xs text-wine-500 font-bold block mb-1">Linked Issue Cluster</span>
            {complaint.issueClusterId ? (
              <Link
                to={`/issues/${complaint.issueClusterId.publicIssueId || complaint.issueClusterId._id}`}
                className="flex items-center justify-between text-sm font-bold text-burgundy-800 hover:underline"
              >
                <span className="flex items-center gap-1.5 truncate">
                  <Layers className="w-4 h-4 text-peach-700 shrink-0" />
                  {complaint.issueClusterId.publicIssueId}
                </span>
                <span className="text-xs bg-peach-100 px-2 py-0.5 rounded-lg text-burgundy-900 font-bold border border-peach-200">
                  {complaint.issueClusterId.affectedCount || 1} affected
                </span>
              </Link>
            ) : (
              <span className="text-xs text-wine-500 font-medium">Standalone report</span>
            )}
          </div>

          <div className="p-4 rounded-2xl bg-cream-50 border border-cream-300 shadow-sm">
            <span className="text-xs text-wine-500 font-bold block mb-1">Contact Protocol</span>
            <span className="text-xs font-bold text-burgundy-800 flex items-center gap-1.5">
        ...
        <div className="p-5 rounded-2xl bg-cream-50 border border-cream-300 shadow-sm">
          ...
          <p className="text-sm text-wine-900 leading-relaxed whitespace-pre-wrap font-medium">
        ...
        {complaint.aiAnalysis && (
          <div className="p-4 rounded-2xl bg-peach-50 border border-peach-200 text-xs shadow-sm">
            <div className="flex items-center gap-1.5 text-burgundy-900 font-bold mb-1">
              <Sparkles className="w-4 h-4 text-peach-600" /> AI Triage Summary:
            </div>
            <p className="text-wine-800 font-medium">{complaint.aiAnalysis.summary}</p>
          </div>
        )}
        ...
        <div className="mt-8 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-bold text-wine-900 flex items-center gap-2">
              <MessageSquare className="w-5 h-5 text-burgundy-700" />
              Direct Anonymous Conversation Thread
            </h3>
            <span className="text-xs text-burgundy-800 font-bold font-mono">🔒 Verified Anonymous</span>
          </div>
