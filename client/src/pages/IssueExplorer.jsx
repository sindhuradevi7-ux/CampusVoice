...
export const IssueExplorer = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 pb-16">
      ...
      {/* Issue Clusters Grid */}
      {loading ? (
        <div className="p-16 text-center text-slate-500 text-sm glass-panel rounded-2xl">
          Loading active campus issue clusters...
        </div>
      ) : issues.length === 0 ? (
        <div className="p-16 text-center glass-panel rounded-2xl border border-slate-800">
          <Layers className="w-10 h-10 text-slate-600 mx-auto mb-2" />
          <h3 className="text-base font-semibold text-slate-200">No issue clusters found</h3>
            ...
            >
              ...
              <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between gap-2">
                <div className="flex items-center gap-1 text-xs">
                  ...
                  <span className="font-bold text-emerald-300 text-sm">{issue.affectedCount}</span>
                  <span className="text-slate-400 text-[11px]">affected</span>
                </div>

                {issue.status === 'Resolved' || issue.status === 'Closed' ? (
                  <span className="text-[11px] text-emerald-400 font-semibold px-2 py-1 bg-emerald-950/60 rounded-lg border border-emerald-500/30">
                    ✓ Resolved
                  </span>
                ) : issue.isSupportedByMe ? (
                  <button
                    disabled
                    className="px-3 py-1.5 bg-emerald-950/70 border border-emerald-500/40 text-emerald-300 rounded-xl text-xs font-semibold flex items-center gap-1 cursor-default"
                  >
