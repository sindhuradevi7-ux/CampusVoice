...
function createEffectComponent(definition: EffectDefinition) {
  return function EffectComponent(props: NeuformBatchEffectProps) {
    const variantKey = props.variant
      ? (props.variant.replace(/-([a-z])/g, (_, c) => c.toUpperCase()) as keyof typeof EFFECTS)
      : undefined;
    const activeDef = (variantKey && EFFECTS[variantKey]) || definition;
    return <NeuformBatchEffect {...props} definition={activeDef} />;
  };
}
